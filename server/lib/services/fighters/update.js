...
export default class FighterUpdate extends ServiceBase {
  static validationRules = {
    id: ['required', 'uuid'],
    competitionId: [
      { required_if: { 'data/sex': 'man' } },
      { required_if: { 'data/sex': 'woman' } },
      'uuid',
    ],
  ...
  async execute({ id, competitionId, recalculate, data }) {
    ...
    if (needUpdateCategory && recalculate) {
      const oldCategories = await Category.scope('cardsWithFighters').findAll({
        where: { '$Cards.Fighter.id$': id },
      });

      const cards = oldCategories.flatMap((c) => c.Cards).filter((card) => card.Fighter.id === id);
