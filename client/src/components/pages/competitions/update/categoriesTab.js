...
const aggregateBy = (array, by) => {
  return array.reduce((acc, c) => {
    ...
    index !== -1 ? acc[index].children.push(c) : acc.push({ [by]: c[by], children: [c] });
    return acc;
  }, []);
};
...
function createAccordionData(sections) {
  return sections.map((section) => {
    return {
      ...
      children: aggregateBy(section.linked.categories, 'sex')?.reduce((acc, c) => {
        section.type === 'full'
          ? acc.push(
              {
                sex: c.sex,
...
function CategoriesTable({ categories, sectionId, sex, group }) {
  ...
  const handleClick = (_, id) => {
    setSelected(selected.includes(id) ? without(selected, id) : [...selected, id]);
  };
  ...
  const onConfirmModal = () => {
    dispatch(
      bulkDelete({ id: selected }, showSuccess('Categories has been successfully deleted.'))
    );
